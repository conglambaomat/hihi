...
Scans .cursor/skills/ directory to build catalog at runtime.
...
def discover_skills(skills_dir: Path) -> dict:
    """Scan .cursor/skills/ and build skill catalog from SKILL.md files."""
    skills = {}
    categories = {}
...
def main():
    # Find .cursor/skills directory
    script_path = Path(__file__).resolve()
    # .cursor/skills/ck-help/scripts/ck-help.py -> .cursor/skills
    skills_dir = script_path.parent.parent.parent

    if not skills_dir.exists():
        print("Error: .cursor/skills/ directory not found.")
        sys.exit(1)
    ...
    if not data["commands"]:
        print("No skills found in .cursor/skills/")
        sys.exit(1)
