...
print(json.dumps({'status':d.get('status'),'summary':d.get('summary'),'coverage':sp.get('coverage_status'),'gate':sp.get('final_answer_gate_status'),'findings_tools':[f.get('tool') for f in d.get('findings',[]) if isinstance(f,dict) and f.get('type')=='tool_result'],'missing':(sp.get('final_answer_gate') or {}).get('missing_evidence'),'search_cov': next((f.get('result',{}).get('coverage_matrix') for f in d.get('findings',[]) if isinstance(f,dict) and f.get('tool')=='search_logs'),{})},ensure_ascii=False,indent=2))
