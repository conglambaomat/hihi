...
Wraps the malwoverview Python package and provides free threat intelligence
using abuse.ch services (MalwareBazaar, URLhaus, Feodo Tracker) plus
optional ThreatFox enrichment when an Auth-Key is configured.
...
URLHAUS_AUTH_KEY = os.environ.get("URLHAUS_AUTH_KEY", ABUSECH_AUTH_KEY).strip()
MALWAREBAZAAR_AUTH_KEY = os.environ.get("MALWAREBAZAAR_AUTH_KEY", ABUSECH_AUTH_KEY).strip()
THREATFOX_AUTH_KEY = os.environ.get("THREATFOX_AUTH_KEY", ABUSECH_AUTH_KEY).strip()


def _build_ssl_context() -> ssl.SSLContext | None:
    """Prefer certifi CA roots when available for stable TLS on Windows."""
    try:
        import certifi
...
def _urlopen(req: urllib.request.Request, timeout: int = TIMEOUT):
    if _SSL_CONTEXT is not None:
        return urllib.request.urlopen(req, timeout=timeout, context=_SSL_CONTEXT)
    return urllib.request.urlopen(req, timeout=timeout)


def _threatfox_auth_required(ioc: str, detail: str | None = None) -> dict:
    message = (
        "ThreatFox now requires an Auth-Key for live API searches. "
        "Set THREATFOX_AUTH_KEY or ABUSECH_AUTH_KEY in the MCP server environment "
        "to enable it."
    )
    if detail:
...
def _abusech_auth_required(source: str, field_name: str, field_value: str, env_hint: str) -> dict:
    return {
        ...
        "manual": True,
        "message": (
            f"{source} now requires an Auth-Key for live API searches. "
            f"Set {env_hint} or ABUSECH_AUTH_KEY in the MCP server environment to enable it."
        ),
        "portal": "https://auth.abuse.ch/",
    }


def _normalize_threatfox_result(tf_result: dict, found_payload: dict | None = None) -> dict:
    """Normalize ThreatFox output so auth / error states remain visible."""
    if tf_result.get("query_status") == "ok" and tf_result.get("data"):
        payload = dict(found_payload or {})
        payload.setdefault("found", True)
        return payload
    if tf_result.get("query_status") == "auth_required":
        return tf_result
...
def _normalize_malwarebazaar_result(mb_result: dict, found_payload: dict | None = None) -> dict:
    """Normalize MalwareBazaar output so auth / error states remain visible."""
    if mb_result.get("query_status") == "ok" and mb_result.get("data"):
        payload = dict(found_payload or {})
        payload.setdefault("found", True)
        return payload
    if mb_result.get("query_status") == "auth_required":
        return mb_result
    if mb_result.get("error"):
        return {
            "found": False,
            "source": "MalwareBazaar",
            "status": "error",
            "error": mb_result.get("error"),
        }
    return {"found": False}


def _has_degraded_sources(sources: dict) -> bool:
    """Return True when any source is auth-gated or errored."""
    for source_result in sources.values():
        if not isinstance(source_result, dict):
            continue
        if source_result.get("query_status") == "auth_required":
            return True
...
def _query_malwarebazaar_hash(hash_value: str) -> dict:
    ...
    raw = _http_post_form(MALWAREBAZAAR_API, {
        "query": "get_info",
        "hash": hash_value,
    }, extra_headers={"Auth-Key": MALWAREBAZAAR_AUTH_KEY})
    try:
...
def _query_urlhaus_url(url: str) -> dict:
    ...
    if not URLHAUS_AUTH_KEY:
        return _abusech_auth_required("URLhaus", "url", url, "URLHAUS_AUTH_KEY")
    raw = _http_post_form(
        f"{URLHAUS_API}/url/",
        {"url": url},
        extra_headers={"Auth-Key": URLHAUS_AUTH_KEY},
    )
    try:
        return json.loads(raw)
    except json.JSONDecodeError:
...
def _query_urlhaus_host(host: str) -> dict:
    ...
    if not URLHAUS_AUTH_KEY:
        return _abusech_auth_required("URLhaus", "host", host, "URLHAUS_AUTH_KEY")
    raw = _http_post_form(
        f"{URLHAUS_API}/host/",
        {"host": host},
        extra_headers={"Auth-Key": URLHAUS_AUTH_KEY},
...
def _query_threatfox_ioc(ioc: str) -> dict:
    ...
    if not THREATFOX_AUTH_KEY:
        return _threatfox_auth_required(ioc)
    try:
        body = json.dumps({
            "query": "search_ioc",
            "search_term": ioc,
        ...
        if exc.code in {401, 403}:
            return _threatfox_auth_required(ioc, f"HTTP {exc.code}")
        return {"error": f"ThreatFox HTTP error: {exc.code}"}
...
def malwoverview_hash_lookup(hash_value: str) -> str:
    ...
    try:
        ...
        results["sources"]["threatfox"] = _normalize_threatfox_result(
            tf_result,
            {"found": True, "entries": tf_result.get("data", [])[:5]},
        )
        ...
        if found_anywhere:
            results["verdict"] = "KNOWN MALWARE"
        elif _has_degraded_sources(results["sources"]):
            results["verdict"] = "PARTIAL - authenticated threat intel sources unavailable"
        else:
            results["verdict"] = "NOT FOUND in free databases"
...
def malwoverview_domain_check(domain: str) -> str:
    ...
    try:
        ...
        results["sources"]["threatfox"] = _normalize_threatfox_result(
            ...
            {
                "found": True,
                "ioc_count": len(tf_result.get("data", [])),
                "entries": [
                    {
                        "ioc": e.get("ioc"),
                    ...
                    for e in tf_result.get("data", [])[:10]
                ],
            },
        )
        ...
        if found_anywhere:
            results["verdict"] = "SUSPICIOUS/MALICIOUS"
        elif _has_degraded_sources(results["sources"]):
            results["verdict"] = "PARTIAL - authenticated threat intel sources unavailable"
        else:
            results["verdict"] = "NOT FOUND in free databases"
...
def malwoverview_url_check(url: str) -> str:
    ...
    try:
        ...
        if found_anywhere:
            results["verdict"] = "MALICIOUS/SUSPICIOUS"
        elif _has_degraded_sources(results["sources"]):
            results["verdict"] = "PARTIAL - authenticated threat intel sources unavailable"
        else:
            results["verdict"] = "NOT FOUND in free databases"
...
def malwoverview_sample_download_info(hash_value: str) -> str:
    ...
    signature, file type, and intelligence data.
    ...
    try:
        ...
        if mb_result.get("query_status") == "auth_required":
            results["found"] = False
            results["source_status"] = mb_result
            return json.dumps(results, indent=2, default=str)
...
def malwoverview_triage(file_path: str) -> str:
    ...
    ThreatFox for matches when Auth-Keys are configured.
    ...
    try:
        ...
        entry = mb_result["data"][0] if mb_result.get("query_status") == "ok" and isinstance(mb_result.get("data"), list) else mb_result.get("data")
        results["sources"]["malwarebazaar"] = _normalize_malwarebazaar_result(
            mb_result,
            {
                "found": True,
                "signature": entry.get("signature") if isinstance(entry, dict) else None,
                "file_type": entry.get("file_type") if isinstance(entry, dict) else None,
                "tags": entry.get("tags") if isinstance(entry, dict) else None,
                "first_seen": entry.get("first_seen") if isinstance(entry, dict) else None,
                "intelligence": entry.get("intelligence") if isinstance(entry, dict) else None,
            },
        )
        ...
        if (
            not results["sources"]["malwarebazaar"].get("found")
            and results["sources"]["malwarebazaar"].get("query_status") != "auth_required"
        ):
