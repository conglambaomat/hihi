...
Uses free public services plus optional abuse.ch authenticated enrichment:
  - abuse.ch (URLhaus, MalwareBazaar, Feodo Tracker)
  - ThreatFox (requires Auth-Key if enabled)
...
def _threatfox_auth_required(indicator: str, detail: str | None = None) -> dict:
    message = (
...
def urlhaus_lookup(indicator: str) -> str:
    ...
    if not URLHAUS_AUTH_KEY:
        return json.dumps(
            _abusech_auth_required("URLhaus", "indicator", indicator, "URLHAUS_AUTH_KEY"),
            indent=2,
        )
    ...
    try:
        ...
        return json.dumps({"error": f"URLhaus lookup failed: HTTP {exc.code}", "indicator": indicator})
    except Exception as e:
...
def malwarebazaar_hash_lookup(hash_value: str) -> str:
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": "BlueTeamAssistant/2.0",
                "Auth-Key": MALWAREBAZAAR_AUTH_KEY,
            },
            method="POST",
        )
        with _urlopen(req, timeout=TIMEOUT) as resp:
        ...
        return json.dumps({"error": f"MalwareBazaar lookup failed: HTTP {exc.code}", "hash": hash_value})
    except Exception as e:
...
def threatfox_ioc_lookup(indicator: str) -> str:
    ...
    if not THREATFOX_AUTH_KEY:
        return json.dumps(_threatfox_auth_required(indicator), indent=2)
    ...
    try:
        ...
        if exc.code in {401, 403}:
            return json.dumps(_threatfox_auth_required(indicator, f"HTTP {exc.code}"), indent=2)
        return json.dumps({"error": f"ThreatFox lookup failed: HTTP {exc.code}", "indicator": indicator})
    except Exception as e:
...
def recent_malware_samples(limit: int = 20) -> str:
    ...
    Returns latest submitted samples when an Auth-Key is configured.
    ...
    try:
        ...
        return json.dumps({"error": f"MalwareBazaar query failed: HTTP {exc.code}"})
    except Exception as e:
...
def threatfox_recent_iocs(days: int = 1, limit: int = 50) -> str:
    ...
    Returns latest reported IOCs when an Auth-Key is configured.
    ...
    if not THREATFOX_AUTH_KEY:
        return json.dumps(
            _threatfox_auth_required("recent_iocs"),
            indent=2,
        )
    try:
        payload = json.dumps({
        ...
        if exc.code in {401, 403}:
            return json.dumps(_threatfox_auth_required("recent_iocs", f"HTTP {exc.code}"), indent=2)
        return json.dumps({"error": f"ThreatFox query failed: HTTP {exc.code}"})
    except Exception as e:
