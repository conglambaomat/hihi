...
async def run_playbook(request: Request, playbook_id: str, body: PlaybookRunRequest = PlaybookRunRequest()):
    ...
    try:
        ...
        if isinstance(raw_body, dict):
            if isinstance(raw_body.get("params"), dict):
                params = raw_body["params"]
            else:
                raw_params = {k: v for k, v in raw_body.items() if k != "case_id"}
                if raw_params:
                    params = raw_params
            if raw_body.get("case_id") is not None:
                case_id = raw_body.get("case_id")
