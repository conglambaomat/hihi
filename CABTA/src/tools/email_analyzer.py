...
class EmailAnalyzer:
    ...
    async def analyze(self, email_path: str) -> Dict:
        ...
        try:
            ...
            received_blob = "\n".join(msg.get_all('Received', []))
            sender_ip_match = re.search(r"\b(?:\d{1,3}\.){3}\d{1,3}\b", received_blob)
            email_data['message_id'] = email_data.get('message_id') or header_analysis.get('message_id', '')
            email_data['return_path'] = email_data.get('return_path') or header_analysis.get('return_path', '')
            email_data['sender_ip'] = (
                header_analysis.get('originating_ip')
                or (sender_ip_match.group(0) if sender_ip_match else '')
                or email_data.get('sender_ip', '')
            )
    ...
    async def _extract_email_data(self, msg: email.message.Message) -> Dict:
        ...
        if msg.is_multipart():
            for part in msg.walk():
                ...
                # Extract attachments
                    ...
                    with tempfile.NamedTemporaryFile(delete=False, prefix='aisa-email-', suffix=suffix) as tmp:
                        tmp.write(payload)
                    ...
                    email_data['attachments'].append({
                        ...
                        'type': Path(filename).suffix.lstrip('.').lower(),
                    })
        else:
            try:
        ...
        email_data['body'] = email_data['body_text']
    ...
    async def _analyze_attachments(self, msg: email.message.Message, email_data: Dict) -> List[Dict]:
        ...
        for attachment_info in email_data['attachments'][:3]:
            try:
                tmp_path = attachment_info.get('path')
                if not tmp_path or not Path(tmp_path).exists():
                    for part in msg.walk():
                        if part.get_filename() == attachment_info['filename']:
                            with tempfile.NamedTemporaryFile(delete=False, prefix='aisa-email-', suffix=Path(attachment_info['filename']).suffix) as tmp:
                                tmp.write(part.get_payload(decode=True) or b'')
                                tmp_path = tmp.name
                            attachment_info['path'] = tmp_path
                            break

                if not tmp_path or not Path(tmp_path).exists():
                    raise FileNotFoundError(f"Attachment path missing for {attachment_info['filename']}")
