...
class SandboxOrchestrator:
    ...
    def select_sandbox(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            path = Path(file_path)
            # Use os.path.isfile for the existence/type check so test doubles that
            # patch Path.stat for size simulation do not break the control flow.
    ...
    def _docker_available(self) -> bool:
        if "docker" not in self._available_cache:
            self._available_cache["docker"] = shutil.which("docker") is not None
        return self._available_cache["docker"]

    def _vm_available(self) -> bool:
        if "vm" not in self._available_cache:
            vm_cfg = self._config.get("sandbox", {}).get("vm", {})
            self._available_cache["vm"] = bool(vm_cfg.get("staging_dir"))
        return self._available_cache["vm"]

    def get_sandbox_status(self) -> List[Dict]:
        """Return availability status of all known sandbox environments."""
        docker_available = self._docker_available()
        vm_available = self._vm_available()
        status_list: List[Dict] = [
            {
                ...
                "status": "available" if docker_available else "not_available",
                "detail": "Dynamic detonation via Docker with network isolation."
                if docker_available
                else "Docker is not installed or not available on PATH.",
            },
            {
                "id": "vm",
                "name": "VM Staging",
                ...
                "status": "configured" if vm_available else "not_configured",
                "detail": "VM sandbox staging directory is configured."
                if vm_available
                else "No sandbox.vm.staging_dir configured for VM submission.",
            },
            {
                "id": "local_static",
                ...
                "available": True,
                "status": "available",
                "detail": "Static-only analysis for Office, PDF, and generic files without execution.",
            },
            {
                "id": "cloud_api",
                "name": "Cloud Sandbox Adapters",
                "type": "local",
                "available": bool(self._cloud_adapters),
                "status": "configured" if self._cloud_adapters else "not_configured",
                "detail": f"{len(self._cloud_adapters)} cloud adapter(s) registered."
                if self._cloud_adapters
                else "No cloud sandbox adapters registered.",
            },
        ]
        seen: set = set()
        ...
        if self.mcp_client is not None:
            for name, info in self.mcp_client.get_connection_status().items():
                ...
                status_list.append({
                    ...
                    "status": "healthy" if info.get("connected", False) else "optional",
                    "tool_count": info.get("tool_count", 0),
                    "detail": f"{info.get('tool_count', 0)} tool(s) visible from MCP server.",
                })
        ...
        for name, adapter in self._cloud_adapters.items():
            if name not in seen:
                ...
                status_list.append({
                    ...
                    "detail": "Cloud sandbox adapter is registered and ready for submissions.",
                })
