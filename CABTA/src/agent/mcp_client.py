    ...
    * **Store integration** -- connection metadata is persisted via the
    ...
    def _persist_connection(self, name: str, connection: MCPConnection) -> None:
        ...
        try:
            ...
            config_data = connection.config.to_dict()
            existing_config = None
            try:
                for saved in self.store.list_mcp_connections():
                    if saved.get("name") == name:
                        existing_config = saved.get("config_json")
                        break
            except Exception:
                existing_config = None

            if isinstance(existing_config, dict):
                # Preserve persisted-only fields such as auth tokens that are
                # intentionally omitted from ``to_dict()`` for log safety.
                merged = dict(existing_config)
                merged.update({k: v for k, v in config_data.items() if v is not None})
                config_data = merged
